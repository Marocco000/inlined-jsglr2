...
public class JSGLR2BenchmarkExternal extends JSGLR2Benchmark<String, StringInput> {
    ...
    @Param({ "standard", "elkhound", "recovery", "incremental", "recoveryElkhound", "recoveryIncremental", "inlinedIncremental" }) JSGLR2Variant.Preset variant;
    ...
    @Override protected Object action(Blackhole bh, StringInput input) throws ParseException {
        if(implode)
            return parser.parseUnsafe(input.content);
        else
            return parser.parseUnsafe(input.content);
    }
